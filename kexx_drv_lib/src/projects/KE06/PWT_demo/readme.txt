...
configure PWT module and measure input clock on PWTIN[0](PTD5 pin). Simulate 1KHz clock using RTC moudle on PTD0 pin.
the code must be runned in FEE mode with exteral 8MHz crystal, and TIMER_CLK is 40MHz.
the code is only for input clock is from 100Hz~10KHz test.
...
- projects/ke06/PWT_demo.c    Main program
...
- Short PTD5 and PTD0 using 1KHz waveform or Connect input pulse(100~10KHz is recommended) to PTD5 pin
- Run the example
